...
PipelineManager::PipelineManager(const vkb::Device& device, float width, float height, VkRenderPass renderPass, std::shared_ptr<PipelineLayoutManager> pipelineLayoutManager)
    : _device(device)
    , _width(width)
    , _height(height)
...
std::shared_ptr<Pipeline> PipelineManager::getPipeline(const std::vector<VertexAttributeDescription>& descriptions, const std::vector<VkDescriptorType>& types)
{
