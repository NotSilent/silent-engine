...
MaterialManager::MaterialManager(const vkb::Device& device, std::shared_ptr<PipelineManager> pipelineManager, std::shared_ptr<DescriptorSetManager> descriptorSetManager)
    : _device(device)
...
std::shared_ptr<Material> MaterialManager::getMaterial(const std::vector<VertexAttributeDescription>& descriptions, const std::vector<VkDescriptorType>& types, std::vector<std::shared_ptr<Texture>>& textures)
{
