...
Pipeline::Pipeline(const vkb::Device& device, float width, float height, VkRenderPass renderPass, const std::vector<VertexAttributeDescription>& attributeDescriptions, std::shared_ptr<PipelineLayout> layout)
    : _device(device)
    ...
    , _layout(layout)
{
    ...
    if (vkCreateGraphicsPipelines(device.device, nullptr, 1, &createInfo, nullptr, &_pipeline) != VK_SUCCESS) {
...
bool Pipeline::isCompatible(const std::vector<VertexAttributeDescription>& attributeDescriptions, std::shared_ptr<PipelineLayout> layout)
{
    if(_layout != layout) {
        return false;
    }
...
VkPipelineShaderStageCreateInfo Pipeline::createPipelineShaderStageCreateinfo(const VkShaderStageFlagBits shaderStage, const VkShaderModule module)
{
    return {
