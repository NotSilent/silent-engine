...
void ImageManager::addImage(const std::string& path)
{
    ...
    //_images[path] = std::make_shared<Image>(vertexCount, vertexBuffer, indexCount, indexBuffer);
}

std::weak_ptr<Image> ImageManager::getImage(const std::string& path)
{
    return {};
}
